...
## 2026-07-09 - 评测语义从二分类扩展为期望类型

### 变更内容
将 golden dataset 的题目语义从单纯 `answerable` 二分类扩展为 `expectation` 类型，当前支持 `direct_answer`、`corrective_answer`、`insufficient_detail` 和 `out_of_scope_refusal`。其中 `corrective_answer` 用于问题前提与证据相矛盾的陷阱题，要求模型引用证据纠正前提；`insufficient_detail` 用于论文相关但缺少具体细节的题，要求模型说明细节未提供并引用相关证据；只有 `out_of_scope_refusal` 才表示标准拒答。评测指标同步改为按期望类型区分回答、引用、检索和拒答分母，并在报告中输出每个 case 的 `expectation`。同时移除答案生成前“问题与 chunk 词项无交集就拒答”的过滤，只保留 `min_score` 阈值，避免中文问题检索英文论文时被词面交集误拦。

### 关键原因
上次评测暴露出一个核心问题：系统把“没有相关证据”和“证据能证明问题前提错误或细节缺失”都当成证据不足，从而把应该纠错或说明细节缺失的题错误评为拒答。引入 `expectation` 后，评测可以区分直接回答、纠错回答、细节缺失说明和语料外拒答，后续优化也能更准确定位问题来自检索、生成还是拒答策略。移除词项交集过滤，是因为当前论文多为英文，而用户问题常为中文夹英文术语，词面交集不是可靠的 RAG 拒答条件，容易在语义检索已经找到相关证据时提前拦截。

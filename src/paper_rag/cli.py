...
def eval_command(
    ...
    case_ids: Annotated[
        ...
        typer.Option(
            "--case-id",
            help="只评测指定 case ID；可重复传入，省略时运行全量评测。",
        ),
    ] = None,
    judge: Annotated[
        bool,
...
def eval_judge_command(
    input_report_path: Annotated[
        Path,
        typer.Argument(exists=True, dir_okay=False, help="已有 eval JSON report 路径。"),
    ],
    dataset_path: Annotated[
        Path,
        typer.Option("--dataset", help="用于读取 evaluation_requirements 的 golden dataset。"),
    ] = Path("eval/datasets/golden.jsonl"),
    case_ids: Annotated[
        list[str] | None,
        typer.Option(
            "--case-id",
            help="只 judge 指定 case ID；可重复传入，省略时处理 report 中全部 case。",
        ),
    ] = None,
    llm_model: Annotated[
        str | None,
        typer.Option("--chat-model", "--llm-model", help="judge 使用的对话模型名称。"),
    ] = None,
    chat_source: Annotated[
        str | None,
        typer.Option("--chat-source", help="judge 使用的对话模型来源。"),
    ] = None,
    report_json: Annotated[
        Path | None,
        typer.Option("--report-json", help="judge-only JSON report 输出路径。"),
    ] = None,
) -> None:
    """只对已有 eval report 中的答案重新运行 LLM judge。"""
    settings = load_settings()
    registry = get_component_registry(settings)
    try:
        answer_generator = _make_answer_generator(
        ...
        judge_client = _make_judge_client(answer_generator=answer_generator)
        result = run_judge_only(
            JudgeOnlyConfig(
                input_report_path=input_report_path,
                dataset_path=dataset_path,
                case_ids=case_ids or [],
                judge_source=chat_source or settings.chat_source,
                judge_model=llm_model or settings.llm_model,
            ),
            judge_client=judge_client,
        )
    except (PaperRagError, ValueError) as exc:
    ...
    typer.echo(_format_judge_only_result(result.report))
    if report_json is not None:
        try:
            written_path = write_judge_only_report(result, report_json)
        except OSError as exc:
            raise typer.BadParameter(f"JSON report 写入失败: {exc}") from exc
        typer.echo(f"JSON report: {written_path}")
...
def _format_judge_only_result(report: dict) -> str:
    """把 judge-only report 渲染为 CLI 可读文本。"""
    summary = report.get("summary", {})
    judge_summary = summary.get("judge", {})
    failed_case_ids = judge_summary.get("failed_case_ids") or []
    lines = [
        "Judge-only 运行:",
        f"源报告: {report.get('run', {}).get('judge', {}).get('input_report_path', '')}",
        f"样本数: {summary.get('case_count', 0)}",
        (
            "Judge 通过率: "
            f"{judge_summary.get('passed_count', 0)}/"
            f"{judge_summary.get('case_count', 0)} "
            f"({float(judge_summary.get('pass_rate', 0.0)):.2%}), "
            f"平均分={float(judge_summary.get('average_score', 0.0)):.2f}, "
            f"错误数={judge_summary.get('error_count', 0)}"
        ),
        "失败 judge 样本:",
    ]
    if failed_case_ids:
...
def _make_judge_client(*, answer_generator: Generator):
    """复用答案生成器的聊天客户端作为 judge client。"""

    chat_client = getattr(answer_generator, "chat_client", None)

    if chat_client is None or not hasattr(chat_client, "complete"):
        raise ValueError("启用 --judge 时需要使用外部聊天生成器，不能使用本地抽取式生成器。")

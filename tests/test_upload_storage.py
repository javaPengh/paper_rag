...
def test_local_upload_storage_saves_pdf_under_tenant_root(tmp_path: Path) -> None:
    root_dir = tmp_path / "test_uploads" / uuid4().hex
...
def test_local_upload_storage_sanitizes_tenant_path_segment(tmp_path: Path) -> None:
    root_dir = tmp_path / "test_uploads" / uuid4().hex
    storage = LocalUploadStorage(root_dir)
...
def test_local_upload_storage_rejects_non_pdf_extension(tmp_path: Path) -> None:
    storage = LocalUploadStorage(tmp_path / "test_uploads" / uuid4().hex)
...
def test_local_upload_storage_rejects_non_pdf_header(tmp_path: Path) -> None:
    storage = LocalUploadStorage(tmp_path / "test_uploads" / uuid4().hex)
...
def test_local_upload_storage_rejects_empty_upload(tmp_path: Path) -> None:
    storage = LocalUploadStorage(tmp_path / "test_uploads" / uuid4().hex)
...
def test_local_upload_storage_rejects_uploads_over_size_limit(tmp_path: Path) -> None:
    storage = LocalUploadStorage(
        tmp_path / "test_uploads" / uuid4().hex,
...
def test_local_upload_storage_rejects_unexpected_content_type(tmp_path: Path) -> None:
    storage = LocalUploadStorage(tmp_path / "test_uploads" / uuid4().hex)
